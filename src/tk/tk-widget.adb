...
package body Tk.Widget is
   ...
   begin
      ...
      if Long_Long_Integer(Length(Source => Options_String)) >
        Long_Long_Integer(Positive'Last) -
          (Long_Long_Integer
             (To_Lower(Item => Place_Type'Image(Value))'Length) +
           Long_Long_Integer(Name'Length) + 3) then
         return;
      end if;
      Append
   ...
   begin
      ...
      if Long_Long_Integer(Length(Source => Options_String)) >
        Long_Long_Integer(Positive'Last) -
          (Long_Long_Integer
             (To_Lower(Item => Justify_Type'Image(Value))'Length) +
           Long_Long_Integer(Name'Length) + 3) then
         return;
      end if;
      Append
      ...
      if Long_Long_Integer(Length(Source => Options_String)) >
        Long_Long_Integer(Positive'Last) -
          (Long_Long_Integer(Extended_Natural'Image(Value.X)'Length) +
           ...
           Long_Long_Integer(Name'Length) + 2) then
         return;
      end if;
      Append
        (Source => Options_String,
   ...
   begin
      ...
      if Long_Long_Integer(Length(Source => Options_String)) >
        Long_Long_Integer(Positive'Last) -
          (Long_Long_Integer(Name'Length) + 38) then
         return;
      end if;
      Append(Source => Options_String, New_Item => " -" & Name);
      if Value < 0 then
   ...
   begin
      if Widgt = Null_Widget then
         return
           (Message_Length => 35, Return_Code => TCL_ERROR,
            Message => "The selected widget is Null_Widget.", Result => 0);
      end if;
      if Command_Name'Length = 0 then
         return
           (Message_Length => 23, Return_Code => TCL_ERROR,
            Message => "Empty the command name.", Result => 0);
      end if;
      if not Name_Is_Valid(Command_Name) then
         return
           (Message_Length => 25, Return_Code => TCL_ERROR,
            Message => "Invalid the command name.", Result => 0);
      end if;
      if Long_Long_Integer(Options'Length) >
        Long_Long_Integer(Natural'Last) -
          (Long_Long_Integer(Tk_Path_Name(Widgt => Widgt)'Length) +
         ...
         return
           (Message_Length => 17, Return_Code => TCL_ERROR,
            Message => "Command too long.", Result => 0);
      end if;
      return
        Tcl_Eval
          (Tcl_Script =>
